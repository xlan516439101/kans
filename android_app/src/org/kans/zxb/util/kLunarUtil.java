...
import org.kans.zxb.R.array;
import org.kans.zxb.R.string;

/**
 * #Lunar# A Util class for Lunar
 * 
 */
public class kLunarUtil {
	public static void test(Context mContext) {
		kLunarUtil mLunarUtil = kLunarUtil.getInstance(mContext);
		Calendar mCalendar = Calendar.getInstance();
		if (mLunarUtil.canShowLunarCalendar()) {
			String str = mLunarUtil.getLunarDateString(mCalendar);
		}
	}
	...
	/**
	 * All days have solar term form 1970.1 to 1936.12 Line represents on year.
	 */
	 ...
	 * 
	 * @param lunarYear
	 *            which lunar year days number to return.
	 ...
	 * 
	 * @param lunarYear
	 *            which lunar year to return.
	 * @return the number of the leapMonth.if hasn't leap month will return 0.
	 */
	 ...
	 * @param gregorianDay
	 * @return int[4],int[0] is luanrYear,int[1] is luanrMonth (index base
	 *         1),int[2] is luanrDay int[3] represent is current month leap
	 *         month,if is leap month,will return LEARP_MONTH,else return
	 *         NORMAL_MONTH
	 */
	public int[] calculateLunarByGregorian(int gregorianYear, int gregorianMonth, int gregorianDay) {
		...
		int offsetDaysFromBaseDate = Math.round(((currentDate.getTime() - baseDate.getTime()) / 86400000.0f));

		int tempLunaryear;
		int daysOfTempLunaryear = 0;
		// start calculator the lunar year.
		// loop use (offsetDaysFromBaseDate - daysOfTempLunaryear) until
		// (offsetDaysFromBaseDate <= 0)
		...
		// if offsetDaysFromBaseDate < 0,tempLunaryear + 1 is the right lunar
		// year.
		for (tempLunaryear = LUNAR_YEAR_BASE; tempLunaryear < LUNAR_YEAR_END && offsetDaysFromBaseDate > 0; tempLunaryear++) {
		...
		// now the value of offsetDaysFromBaseDate equals the day of the lunar
		// year,like:111/365
		// when offsetDaysFromBaseDate <= 0,then tempLunarMonth <= the right
		// lunar month
		// so if offsetDaysFromBaseDate < 0,the previous lunar month is the
		// right lunar month
		// if offsetDaysFromBaseDate = 0,the tempLunarMonth si the right lunar
		// month
		...
		// if offsetDaysFromBaseDate == 0,it says the tempLunarMonth is the leap
		// month
		// But now the value of tempLunarMonth = leapMonth + 1,so we should
		// minus 1.
		...
		// now the value of the offsetDaysFromBaseDate equals the lunar day +
		// 1,like:11/31
		// only plus 1.
	...
	private String getLunarDateString(int lunarYear, int lunarMonth, int lunarDay, int leapMonthCode) {
		...
		// getSpecialWord function, because
		// should show "閏" in TC, "闰" in SC.
		String luanrDateString = cyclicalm(lunarYear) + mLunarTextYear + (leapMonthCode == LEAP_MONTH ? getSpecialWord(LUNAR_WORD_RUN) : "") + mMonthNumberArray[lunarMonth - 1] + mLunarTextMonth + getLunarDayString(lunarDay);
	 ...
	 * 
	 * @param calendar
	 *            The Gregorian date to be decrease or increase.
	 * @param lunarMonth
	 *            decrease or increase happed in which lunar month.(ignore leap
	 *            month)
	 * @param lunarDay
	 *            decrease or increase happed in which lunar day.
	 * @param operatorType
	 * @return The Gregorian date that has been decreaseed or increased a lunar
	 *         year's time
	 */
	public Calendar decreaseOrIncreaseALunarYear(Calendar calendar, int lunarMonth, int lunarDay, int operatorType) {
		...
		while (true) {
			 ...
			 * the date.
			 */
			if ((operatorType == INCREASE_A_LUANR_YEAR) && (lunarDates[1] == lunarMonth) && (lunarDates[2] < lunarDay)) {
			 ...
			 * the date.
			 */
			if ((operatorType == DECREATE_A_LUANR_YEAR) && ((lunarDates[0] + 1) == lunarYear) && (lunarDates[1] > lunarMonth)) {
				break;
			}
	 ...
	 * @return null if the day is not the solar term, otherwise return the solar
	 *         term name.
	 */
	...
	private String[] getAMonthSolarTermNames(int gregorianMonth) {
		...
		return new String[] { getSolarTermNameByIndex(firstSolarTermIndex), getSolarTermNameByIndex(firstSolarTermIndex + 1) };
	}

	/**
	 * Change given year.month.day to Chinese string. Festival, SolarTerm, or
	 * Chinese number. in this method, the Lunar state is force updated to the
	...
	private String getLunarNumber(int lunarMonth, int lunarDay, boolean isLeapMonth) {
		...
		if (lunarDay == 1) {
			if (isLeapMonth) {
				// / M: If the leapMonthCode is LEAP_MONTH show special word by
				// getSpecialWord function, because should show "閏" in TC, "闰"
				// in SC.
	...
	/**
	 * This Extension is for the festivals display. typically, the TC lunar will
	 * have different text displayed from SC Lunar.
	 */
	 ...
	 * 
	 * @param context
	 *            the context is for looking up via PluginManager
	 */
	private kLunarUtil(Context context) {
	...
	public boolean canShowLunarCalendar() {
		return canShowSCLunar() ? true : false; // //mExt.canShowLunarCalendar();
	}
	 ...
	 * @param lunarMonth
	 * @param lunarDay
	 * @param lunarMonthType
	 *            lunar month type, is leap?
	...
	private static kLunarUtil sInstance;

	public static kLunarUtil getInstance(Context context) {
		if (null == sInstance) {
			sInstance = new kLunarUtil(context.getApplicationContext());
		}
		return sInstance;
	}

	/**
	 * M: Get the special word, If local language is SC, return the needed word
	 * in SC, else return the TC word.
	 * 
	 * @param index
	 *            refer to the special word.
	 * @return the word needed,like: "闰" need by LUNAR_WORD_RUN in SC, "閏" in
	 *         TC.
	 */
	...
	public String formatLunarDateRange(Context context, Time startDate, Time endDate, boolean showDateRange) {
		...
		if (startDate.year != startYear || endDate.year != endYear || startDate.year != endDate.year || startYear != endYear || startYear != currentYear || endYear != currentYear) {
			showYear = true;
		}
		// if the start time and end time is in one day, then show date only.
		if (!showDateRange) {
			return context.getString(R.string.lunar_detail_info_fmt1, (showYear ? startYear + mLunarTextYear : "") + lunarStartDate.get(MONTH) + lunarStartDate.get(MONTH_DAY));
		}
