...
import org.kans.zxb.fragment.ProductClassFragment;
...
public class RouductClassPresenter extends KPresenter<RouductClassPresenter.Ui> implements OnScrollListener {

	private RouductClassAdapter mRouductClassAdapter;
	private Runnable notifyRun;

	public interface Ui extends KUi {
		ProductClassFragment getUi();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mRouductClassAdapter = new RouductClassAdapter();
		updateData();
	}
	...
	public void onStart() {
		super.onStart();
		getUi().getListView().setAdapter(mRouductClassAdapter);
		...
		x.task().run(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if(mProductClasses!=null){
						notifyRun = new Runnable() {
							@Override
							public void run() {
								notifyRun = null;
								mRouductClassAdapter.notifyDataSetChanged(mProductClasses);
							}
						};
	...
	public class RouductClassAdapter extends BaseAdapter implements KListItemCallback {
		...
		public void notifyDataSetChanged(List<ProductClass> mProductClasses) {
			...
			this.mProductClasses.addAll(mProductClasses);
		...
		private void initView(ItemViewHold mViewHold) {
			...
			mViewHold.delButton.setOnClickListener(RouductClassPresenter.this);
			mViewHold.editButton.setTag(mViewHold.mProductClass);
			mViewHold.editButton.setOnClickListener(RouductClassPresenter.this);
